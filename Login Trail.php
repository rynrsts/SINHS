...
<body class="fixed-nav sticky-footer bg-dark" id="page-top">
	...
	<div class="content-wrapper">
		<div class="container-fluid">
			...
			<!-- Breadcrumbs-->
			<ol class="breadcrumb">
				<li class="breadcrumb-item">Login Trail</li>
			</ol>
			<div class="card mb-3">
				<div class="card-header">
					<i class="fa fa-table"></i> Login Trail History</div>
				<div class="card-body">
					<div class="table-responsive">
						<table class="table table-bordered" width="100%" cellspacing="0">
							...
							<tbody>
								<form method="post" action="Login Trail.php">

									<!-- PHP -->
									<?php
										require('connection.php');

										$sqlView = "SELECT * FROM tbl_login";
...
<?php
	...
	if (isset($_POST['delete'])) {
		...
		if (isset($_POST['check'])) {
			...
			for ($i = 0; $i < $cntRec; $i++) {
				$delID = $delRec[$i];
				$sqlDelete = "DELETE FROM tbl_login WHERE id = $delID";
				mysqli_query($link, $sqlDelete);

				$forAlert = true;
			}
			if ($forAlert == true) {
				echo "<script>alert('The login trail/s has/have been deleted.')</script>";
				echo "<script>window.location.assign('Login Trail.php')</script>";
			}
		}
		else {
